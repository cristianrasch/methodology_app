xml.chart(:dateFormat => "dd/mm/yyyy", :hoverCapBorderColor => "2222ff", :hoverCapBgColor => "e1f5ff", :ganttWidthPercent => "82", :ganttLineAlpha => "80", :canvasBorderColor => "024455", :canvasBorderThickness => "0", :gridBorderColor => "4567aa", :gridBorderAlpha => "20") do
  xml.categories(:bgColor => "ffffff", :fontColor => "1288dd", :fontSize => "10") do
    month = Date.today.month
    10.times do |i|
      next_month = i.months.from_now.to_date
      xml.category(:start => l(next_month.at_beginning_of_month), :end => l(next_month.at_end_of_month.to_date), :align => "center", :name => t('date.month_names')[month+i > 12 ? month+i-12 : month+i], :isBold => "1")
    end
  end
